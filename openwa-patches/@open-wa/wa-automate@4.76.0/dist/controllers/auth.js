...
const needsToScan = (waPage) => {
    return (0, rxjs_1.from)(new Promise((resolve) => __awaiter(void 0, void 0, void 0, function* () {
        try {
            yield Promise.race([
                waPage.waitForSelector("canvas", { timeout: 10000 }).catch(() => { }),
                waPage.waitForSelector("[data-ref]", { timeout: 10000 }).catch(() => { })
            ]);
            resolve(false);
        }
        catch (error) {
...
class QRManager {
    constructor(config = null) {
        ...
        this.qrCheck = `(() => { const canvas = document.querySelector("canvas"); if (canvas) { try { const d = canvas.toDataURL("image/png"); if (d && d.length > 200) return d; } catch(e){} } const refEl = document.querySelector("[data-ref]"); if (refEl) return refEl.getAttribute("data-ref"); const parentWithRef = canvas ? canvas.closest("[data-ref]") : null; return parentWithRef ? parentWithRef.getAttribute("data-ref") : false; })()`;
        this.config = config;
    ...
    grabAndEmit(qrData, waPage, config, spinner) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (!this._internalQrPngLoaded && !(typeof qrData === 'string' && qrData.startsWith('data:image/'))) {
                logging_1.log.info("Waiting for internal QR renderer to load");
                const t = yield (0, tools_1.timePromise)(() => waPage.waitForFunction(`window.getQrPng || document.querySelector("canvas") || false`, { timeout: 5000 }).catch(() => {}));
                logging_1.log.info(`Internal QR renderer loaded in ${t} ms`);
                this._internalQrPngLoaded = true;
            }
            try {
                const qrPng = (isLinkCode || (typeof qrData === 'string' && qrData.startsWith('data:image/'))) ? qrData : yield waPage.evaluate(`(() => { if (typeof window.getQrPng === 'function') return window.getQrPng(); const canvas = document.querySelector("canvas"); return canvas ? canvas.toDataURL("image/png") : null; })()`).catch(() => qrData);
                if (qrPng) {
                    qrEv.emit(qrPng);
                ...
                //@ts-ignore
                const lr = yield waPage.evaluate("window.launchres").catch(() => null);
                console.log(lr);
    ...
    smartQr(waPage, config, spinner) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (isAuthed)
                return true;
            const _hasDefaultStateYet = yield waPage.evaluate("!!(window.Store && window.Store.State && window.Store.State.Socket)").catch(() => false);
            if (!_hasDefaultStateYet) {
                //expecting issue, take a screenshot then wait a few seconds before continuing
                yield (0, tools_1.timeout)(2000);
            }
            return new Promise((resolve) => __awaiter(this, void 0, void 0, function* () {
                ...
                const intervalId = setInterval(() => __awaiter(this, void 0, void 0, function* () {
                    ...
                    try {
                        const isAuthed = yield waPage.evaluate("!!window.WA_AUTHENTICATED || (document.getElementsByClassName('app')[0] && document.getElementsByClassName('app')[0].attributes && !!document.getElementsByClassName('app')[0].attributes.tabindex) || (document.getElementsByClassName('two')[0] && document.getElementsByClassName('two')[0].attributes && !!document.getElementsByClassName('two')[0].attributes.tabindex)").catch(() => false);
                        if (isAuthed) {
                            gotResult = true;
                            clearInterval(intervalId);
                            spinner === null || spinner === void 0 ? void 0 : spinner.succeed("Authenticated successfully!");
                            return resolve(true);
                        }
                        const currentQr = yield waPage.evaluate(this.qrCheck).catch(() => null);
                        if (currentQr) {
                            yield this.grabAndEmit(currentQr, waPage, config, spinner);
                        }
                    } catch (err) {}
                }), 2000);
